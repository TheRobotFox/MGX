...
#define __MGX_T_FORWARD_RECURSIVE_CALL(FUNCTION, ARG, M, L, ...) __MGX_DEFER_n(L, FUNCTION)\
											(__MGX_DEFER2(__MGX_T_FORWARD_RECURSIVE_INDIRECT)() (M, MGX_DEC(MGX_DEC(L)), __VA_ARGS__), ARG)

#define __MGX_T_FORWARD_RECURSIVE_ARRAY(M, L, ...) __MGX_T_FORWARD_RECURSIVE_ARRAY_ __MGX_LPAREN M, L, __MGX_T_CAT(__MGX_T_EXTRACT_, MGX_ARG_FIRST(__VA_ARGS__)) ,  __VA_ARGS__ __MGX_RPAREN
#define __MGX_T_FORWARD_RECURSIVE_ARRAY_(M, L, T, S, ...) __MGX_T_FORWARD_RECURSIVE_CALL( M ## _ARRAY, S, M, L, T MGX_ARG_TAIL_OPT(__VA_ARGS__) )


#define __MGX_T_FORWARD_RECURSIVE_(M, L, ...) MGX_IF(__MGX_T_IS_ARRAY(MGX_ARG_FIRST(__VA_ARGS__)))\
				   (__MGX_T_FORWARD_RECURSIVE_ARRAY(M, L, __VA_ARGS__), MGX_IF(__MGX_T_IS_PTR(MGX_ARG_FIRST(__VA_ARGS__))) \
				   (__MGX_T_FORWARD_RECURSIVE_ARRAY(M, L, __VA_ARGS__), MGX_IF(__MGX_T_IS_COMP(MGX_ARG_FIRST(__VA_ARGS__))) \
...
#define __MGX_T_WHILE(c, ...) __MGX_T_EVAL(__MGX_WHILE(c, __VA_ARGS__))
...
#define __MGX_T_GET_VAR_PTR(A) (*A)
...
#define __MGX_T_NOT_COMP(...) MGX_NOT(__MGX_T_CAT_(__MGX_T_IS_COMP(MGX_ARG_FIRST(__VA_ARGS__))))
#define __MGX_T_FORWARD(M, R, T) M, __MGX_T_EXPAND(__MGX_T_CAT(M##_, __MGX_T_CAT_(__MGX_T_POSTFIX, T)) __MGX_T_FORWARD_ARGLIST(R,T) )
//#define __MGX_T_FORWARD_(P, M, R, T) __MGX_T_EXPAND(M##_##P __MGX_T_FORWARD_ARGLIST_(R,T) )

#define __MGX_T_FORWARD_ARGLIST(R,T) ( R, __MGX_CAT_(__MGX_T_EXTRACT_, T) )

#define __MGX_T_EXTRACT_ARRAY(T, S) T, S
#define __MGX_T_EXTRACT_PTR(T) T

#define __MGX_T_POSTFIX_PTR(...) PTR
#define __MGX_T_POSTFIX_ARRAY(...) ARRAY

#define MGX_T_VAR(T, name) __MGX_T_WHILE(__MGX_T_NOT_COMP, __MGX_T_FORWARD, __MGX_T_GET_VAR, name, T )

MGX_T(ARRAY(extern static volatile restrict const struct abc*, 3))
MGX_T(PTR(ARRAY(extern static volatile restrict const struct abc*, 3)))
MGX_T(struct abc)
